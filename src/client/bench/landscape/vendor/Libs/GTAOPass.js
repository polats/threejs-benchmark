// VENDORED from three@0.184.0 examples/jsm/postprocessing/GTAOPass.js
// Kept local so we can tune AO quality and patch shader issues without waiting on
// the CDN. Only the import paths were changed from the upstream original:
//   ./Pass.js                      -> three/addons/postprocessing/Pass.js
//   ../shaders/GTAOShader.js       -> ./GTAOShader.js               (vendored)
//   ../shaders/PoissonDenoiseShader.js -> ./PoissonDenoiseShader.js (vendored)
//   ../shaders/CopyShader.js       -> three/addons/shaders/CopyShader.js
//   ../math/SimplexNoise.js        -> three/addons/math/SimplexNoise.js
// Local patches:
//   - resolutionScale parameter for half-resolution AO.
//   - fog-aware AO fade uniforms from ./GTAOShader.js.
//   - userData.excludeFromGIContribution meshes are skipped in the AO G-buffer
//     (same as skipGTAO) so atmospherics (godrays, clouds) are still drawn in the
//     color pass but don't generate occlusion; the Default output then multiplies
//     AO over the fully composited image.
// Known benign warning: ANGLE/D3D X4000 ("potentially uninitialized variable")
// comes from the dynamic index `poissonDisk[i]` in ./PoissonDenoiseShader.js
// (~L188). Harmless; do not "fix" by dropping `const` - global array initializers
// must be constant expressions and some drivers will then refuse to compile.
import {
	AddEquation,
	Color,
	CustomBlending,
...
class GTAOPass extends Pass {
	...
	constructor( scene, camera, width = 512, height = 512, parameters, aoParameters, pdParameters ) {
		...
		this.resolutionScale = ( parameters && parameters.resolutionScale ) || 1;
	...
	setSize( width, height ) {
		...
		this.gtaoRenderTarget.setSize( w, h );
		this.normalRenderTarget.setSize( w, h );
		this.pdRenderTarget.setSize( w, h );

		this.gtaoMaterial.uniforms.resolution.value.set( w, h );
		...
		this.pdMaterial.uniforms.resolution.value.set( w, h );
	...
	updateGtaoMaterial( parameters ) {
		...
		if ( parameters.fadeStart !== undefined ) {

			this.gtaoMaterial.uniforms.aoFadeStart.value = parameters.fadeStart;

		}

		if ( parameters.fadeEnd !== undefined ) {

			this.gtaoMaterial.uniforms.aoFadeEnd.value = parameters.fadeEnd;
	...
	render( renderer, writeBuffer, readBuffer /*, deltaTime, maskActive */ ) {
		...
		// render normals and depth (honor only meshes; points, lines, skipGTAO and
		// excludeFromGIContribution meshes do not contribute to AO)
	...
	_overrideVisibility() {
		...
		scene.traverse( function ( object ) {
			...
			if ( ( object.isPoints || object.isLine || object.isLine2 || object.userData.skipGTAO || object.userData.excludeFromGIContribution ) && object.visible ) {
